 ...
 * @version    SVN: $Id: RemoveCell.php 112 2013-03-10 21:06:02Z ll77ll $
...
/**
 * Web Table Operation: RemoveCell
 *
 * @category   Web
 * @package    Web_Table
 * @author     Matt Sirocki <mattsirocki@gmail.com>
 ...
 */
class RemoveCell extends \Web\Table\Ajax
{
    /**
     * Handle RemoveCell (Data)
     *
     * @return void
     */
    public function processData()
    {
        // Error Checking
        if ($this->table === null)
            $this->sendFail('Remove Cell', 'You must specify a Table in the request.');
        if (count($this->arguments) != 0)
            $this->sendFail('Remove Cell', 'You must specify exactly zero arguments in the request.');

        // Perform Modification
        $this->row->removeCell($this->cell->index);
    ...
    public function processForm()
    {
        ...
        // Construct Message
        $message = <<<HTML
            Please confirm that you wish to remove the Cell
...
HTML;
        ...
        $note = <<<HTML
            <p>
                ...
                (<code>#{$this->cell->index}</code>)</b> from the
                Row <b>{$this->row->name}
