 ...
 * @version    SVN: $Id: AppendItem.php 112 2013-03-10 21:06:02Z ll77ll $
...
/**
 * Web Table Operation: AppendItem
 *
 * @category   Web
 * @package    Web_Table
 * @author     Matt Sirocki <mattsirocki@gmail.com>
 ...
 */
class AppendItem extends \Web\Table\Ajax
{
    /**
     * Handle AppendItem (Data)
     *
     * @return void
     */
    public function processData()
    {
        // Error Checking
        if ($this->table === null)
            $this->sendFail('Append Item', 'You must specify a Table in the request.');
        if (count($this->arguments) != 3)
            $this->sendFail('Append Item', 'You must specify exactly three arguments in the request.');
        if (!is_string($name = $this->arguments[0]))
            $this->sendFail('Append Item,', 'The new Row name, the first argument, must be a string.');
        if (!is_string($text = $this->arguments[1]))
            $this->sendFail('Append Item', 'The new Item text, the second argument, must be a string.');

        // Extract Arguments
        $checkable = $this->arguments[2] === 'on' ? true : false;

        // Perform Modification
        $item = $this->group->appendItem($name, $text, $checkable);
    ...
    public function processForm()
    {
        ...
        // Construct Note
        $note = <<<HTML
            <p>Click <b>Submit</b> to <b>Append</b> a new Item to the
