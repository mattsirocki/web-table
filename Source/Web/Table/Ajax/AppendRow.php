 ...
 * @version    SVN: $Id: AppendRow.php 112 2013-03-10 21:06:02Z ll77ll $
 ...
 */
class AppendRow extends \Web\Table\Ajax
{
    /**
     * Handle AppendRow (Data)
     *
     * @return void
     */
    public function processData()
    {
        // Error Checking
        if ($this->table === null)
            $this->sendFail('Append Row', 'You must specify a Table in the request.');
        if (count($this->arguments) != 1)
            $this->sendFail('Append Row', 'You must specify exactly one argument in the request.');
        if (!is_string($name = $this->arguments[0]))
            $this->sendFail('Append Row', 'The new Row name, the first argument, must be a string.');

        // Perform Modification
        $row = $this->table->appendRow($name);
    ...
    public function processForm()
    {
        ...
        // Construct Note
        $note = <<<HTML
            <p>Click <b>Submit</b> to <b>Append</b> a new Row to the
