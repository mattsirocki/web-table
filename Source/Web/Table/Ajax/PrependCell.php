 ...
 * @version    SVN: $Id: PrependCell.php 112 2013-03-10 21:06:02Z ll77ll $
...
/**
 * Web Table Operation: PrependCell
 *
 * @category   Web
 * @package    Web_Table
 * @author     Matt Sirocki <mattsirocki@gmail.com>
 ...
 */
class PrependCell extends \Web\Table\Ajax
{
    /**
     * Handle PrependCell (Data)
     *
     * @return void
     */
    public function processData()
    {
        // Error Checking
        if ($this->table === null)
            $this->sendFail('Prepend Cell', 'You must specify a Table in the request.');
        if (count($this->arguments) != 1)
            $this->sendFail('Prepend Cell', 'You must specify exactly one argument in the request.');
        if (!is_string($name = $this->arguments[0]))
            $this->sendFail('Prepend Cell', 'The new Cell name, the first argument, must be a string.');

        // Perform Modification
        $cell = $this->row->prependCell($name);
    ...
    public function processForm()
    {
        ...
        $note = <<<HTML
            <p>
                Click <b>Submit</b> to <b>Prepend</b> a new Cell to the
                Row <b>{$this->row->name}
